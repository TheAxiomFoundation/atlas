-- Citation graph across corpus.provisions.
--
...
-- per (source_provision_id, start_offset) via a unique index. Re-running the
...
-- source_provision_id first. We don't enforce this via trigger because the
-- extractor runs as a batch and owns the lifecycle.

CREATE TABLE IF NOT EXISTS corpus.provision_references (
  id                    UUID PRIMARY KEY DEFAULT gen_random_uuid(),

  -- The rule containing the citation (always resolved).
  source_provision_id        UUID NOT NULL REFERENCES corpus.provisions(id) ON DELETE CASCADE,

  -- The rule being cited. `target_citation_path` is always present;
  -- `target_provision_id` is resolved iff that path exists in corpus.provisions.
  ...
  target_provision_id        UUID REFERENCES corpus.provisions(id) ON DELETE SET NULL,
  ...
  -- Char offsets inside `corpus.provisions.body`, suitable for the viewer to
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_provision_references_source_span
  ON corpus.provision_references (source_provision_id, start_offset, end_offset);

-- Outgoing lookups: "what does THIS rule cite?" — primary path for both
-- the viewer's body-render and RuleSpec's import candidates.
CREATE INDEX IF NOT EXISTS idx_provision_references_source
  ON corpus.provision_references (source_provision_id);

-- Incoming lookups: "what cites THIS rule?" — powers a future
-- back-references panel on the rule page, and analytics.
CREATE INDEX IF NOT EXISTS idx_provision_references_target_id
  ON corpus.provision_references (target_provision_id)
...
CREATE INDEX IF NOT EXISTS idx_provision_references_target_path_unresolved
  ON corpus.provision_references (target_citation_path)
  WHERE target_provision_id IS NULL;

-- Count references by kind for telemetry.
CREATE INDEX IF NOT EXISTS idx_provision_references_kind
  ON corpus.provision_references (pattern_kind);


-- ======================================================================
-- RPC: get_provision_references(citation_path)
-- ======================================================================
...
GRANT EXECUTE ON FUNCTION corpus.get_provision_references(text) TO anon;
GRANT EXECUTE ON FUNCTION corpus.get_provision_references(text) TO authenticated;
...
GRANT ALL ON TABLE corpus.provision_references TO postgres, service_role;
GRANT SELECT ON TABLE corpus.provision_references TO anon, authenticated;

ALTER TABLE corpus.provision_references ENABLE ROW LEVEL SECURITY;

-- Public-read policies, mirroring how corpus.provisions is configured: the data
...
CREATE POLICY anon_read ON corpus.provision_references
  FOR SELECT TO anon USING (true);
CREATE POLICY authenticated_read ON corpus.provision_references
